...
//
//   /api/_diag-stripe?token=XXX          → Stripe key, registered webhook endpoints, recent
//                                          checkout.session.completed events and their delivery state
//   /api/_diag-stripe?token=XXX&full=1   → also dumps the metadata of the most recent session
  ...
  try {
    ...
    if (!events.data.length) {
      ...
      if (stuck.length) {
        ...
        out.verdict.push('Recent completed checkouts exist and Stripe has no pending webhooks → the endpoint answered 200. If no mail arrived, the failure is inside the email step: check Vercel logs for "Resend send failed" and run /api/_diag-email.');
      }
