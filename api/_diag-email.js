...
//
//   /api/_diag-email?token=XXX                 → env vars present + Resend account/domain state
//   /api/_diag-email?token=XXX&to=me@mail.it   → also performs a REAL send and returns Resend's raw reply
//
...
module.exports = async (req, res) => {
  ...
  if (domains.status === 401 || domains.status === 403) {
    ...
    if (!list.length) {
      ...
      out.verdict.push(`Key and domain agree: "${fromDomain}" is verified in this account. If mail still does not arrive, the failure is downstream (webhook never fires, or delivery/spam) — run /api/_diag-stripe and add &to= here.`);
    }
  }
